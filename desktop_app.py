...
class ApiClient:
    def _request(self, method, endpoint, json_data=None):
        try:
            ...
            CTkMessageBox(None, "Erro Crítico de Conexão", f"Não foi possível conectar ao servidor.\n{e}", icon="error")
            sys.exit(1)
    ...
    def encontrar_pokemon(self, area_id):
        return self._request("get", f"/mundo/encontrar-pokemon?areaid={area_id}")
    def iniciar_batalha(self, treinador_id, pokemon_nome, nivel):
        payload = {"treinador_id": treinador_id, "pokemon_nome": pokemon_nome, "nivel_encontrado": nivel}
    ...
    def treinar_pokemon(self, treinador_id, id_captura, area_id):
        return self._request("post", f"/treinador/{treinador_id}/treinar/{id_captura}?areaatual={area_id}")
...
class App(ctk.CTk):
    ...
    def handle_escolher_inicial(self, pokemon_nome):
        ...
        try:
            ...
            if pokemon_escolhido:
                CTkMessageBox(self, "Sucesso", f"Você e {pokemon_nome} começam uma nova jornada!", icon="info")
                self.mostrar_frame(TelaGeral)
        except requests.exceptions.HTTPError as e:
            erro_msg = e.response.json().get("detail", "Erro ao escolher o Pokémon.")
            CTkMessageBox(self, "Erro", erro_msg, icon="error")
    ...
    def get_treinador(self, treinador_id):
        return api._request("get", f"/treinador/{treinador_id}")
    ...
    def handle_pc_click(self, clique_info):
        ...
        if not self.pokemon_selecionado_pc:
            ...
            try:
                ...
                CTkMessageBox(pc_win, "Movimento Inválido", erro_msg, icon="error")
            finally:
                self.pokemon_selecionado_pc = None
                self.origem_selecao_pc = None
                if pc_win and pc_win.winfo_exists():
    ...
    def handle_procurar_pokemon(self, area_id):
    ...
    def handle_trocar_pokemon(self, id_captura):
        ...
        try:
            ...
            if "resultado_final" in batalha_atual:
                CTkMessageBox(self, "Batalha Encerrada", batalha_atual["resultado_final"], icon="info")
                self.mostrar_frame(TelaGeral)
        except requests.exceptions.HTTPError as e:
            erro_msg = e.response.json().get("detail", "Não foi possível trocar o Pokémon.")
            CTkMessageBox(self, "Erro de Troca", erro_msg, icon="error")
    ...
    def handle_curar_pokemon(self):
        ...
        if treinador_atual:
            ...
            if resultado:
                CTkMessageBox(self, "Centro Pokémon", resultado["mensagem"], icon="info")

    def mostrar_aviso(self, mensagem):
        CTkMessageBox(self, "Aviso", mensagem, icon="warning")

    def api_get_areas(self):
        return api.get_areas()

# BACKEND e RESET DE ESTADO -------------------------------------------
...
def resetar_gamestate_se_necessario():
    try:
        ...
        estado_limpo = {"batalhas_ativas": [], "treinadores": []}
