...
class TelaGeral(ctk.CTkFrame):
    def __init__(self, master, controller):
        ...
        self.equipe_widgets = []
        self.equipe_container = equipe_container
        self.frame_grid = None
        self.desenhar_equipe()  
        ...
        threading.Thread(target=self._worker_carregar_imagens, daemon=True).start()
    ...
    def _worker_carregar_imagens(self):
        ...
        imagens_carregadas = {}
        for area_id, nome_arquivo in nomes_imagens_fundo.items():
            try:
                image_path = os.path.join(base_dir, "imagens", nome_arquivo)
                imagens_carregadas[area_id] = Image.open(image_path)
            except Exception as e:
                print(f"Erro ao carregar imagem {nome_arquivo}: {e}")
                imagens_carregadas[area_id] = None
        self.after(0, self._on_imagens_carregadas, imagens_carregadas)

    def _on_imagens_carregadas(self, imagens_carregadas):
        if not self.winfo_exists():
            return

        self.imagens_fundo_pil = imagens_carregadas
    ...
    def desenhar_equipe(self):
        ...
        self.frame_grid = ctk.CTkFrame(self.equipe_container, fg_color="transparent")
